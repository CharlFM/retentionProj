...
Clean twitter data by removing accents, hash tags, hyperlinks and some other things as well.
...
Here we calculate the sentiments using the libraries `nlts` and `TextBlob`
...
Export the twitter data results.
...
In this section we will study the variables one-by-one to ensure that we have a clear and full understanding of how the data works. The reason for investigating the variables one-by-one is that we would like to ensure that we understand each variable appropriately and that we treat it accordingly. This was re-enforced especially after we noticed a few strange occurrences in the data.

We will also investigate the variables from a global perspective and remove redundant variables. 
...
This is the median polarity of all the tweets from this individual, calculated using the `TextBlob` library.
...
This is the median sentiment score of all the tweets from this individual, using the `nltk` library.
...
The polarity and the sentiment scores are somewhat correlated, but the difference is a correction for mistakes made by `TextBlob`.
...
It seems that our GBM still remains the leader. The NN does not improve on any KPI. It is also over-fits the train data.
...
Here we can see that the stacked result doesn't improve the overall performance, the stacked model over-fits the training data. The preferred model still remains our GBM.
...
We can see that the automated learning did not prove to be very successful. But still not beating the previously trained GBM.
...
From this it is clear that we will choose the model **gbm_grid_model_51** since it produce a very high **F1-score** and there isn't any proof of over fitting. Here is the model parameters :
...
Here we can see that the main contributors to the low probability is the **pol_med** which equal `-0.0400` (the number in the plot is the standardized value) and the **Group_State** equalling `OR`. There exist some positive contributors as well, such as the **MajorProgramCode** `H`.
...
Here we can see that the main contributors to this prediction is the **MajorProgramCode** and the **sent_med** on the negative side. However, there are a number of positive contributors **Program_Code**, **avePrice** and **Income_Level** are some examples.
...
Here we can see that the main contributors to the high positive probability is the **pol_med** and **avePrice**. There exist some negative contributors as well, the **GroupGradeType** and the **SPR_Product_Type**.
...
4. Creating topics from the twitter data
5. Using the `min`, `max` and `mean` for the twitter features and after testing which is best, the `median` was selected (these were all highly correlated of course)
...
**NOTE** as the sentiment is the most important variable, it should be calculated with caution. If an actual negative tweet is identified as a positive tweet, we will almost certainly classify this observation as being retained and vice versa. This has been observed and required us to review our method.
...
We will also consider the median sentiment.
...
Here we can see that indeed if all the lower income segments were to have attended a more affordable program, the overall portfolio retention would have increased by more than $2\%$. Or in terms of revenue :
...
This states that it would have resulted in more than $5.5\%$ or $216.500\$$ increase in total revenue.
...
Here we can see an unfortunately low impact, by applying this strategy we find an overall increase of $0.0719\%$ increase in the expected profit. This translates to :

```{r}
finDF$maxProfit %>% sum()-finDF$e_profit %>% sum()
```

A small $140\$$ increased overall expected profit.
...
Here we can see that the impact for this individual is big. Opting to reducing the revenue with $1\%$ leads to a $12\%$ increase in retention probability and an $58.79\%$ increase in the expected profit.
