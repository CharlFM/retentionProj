...
twiter_test <- readr::read_csv("2_output/1_data/twitter_results_test.csv") %>%
  mutate(tweetLength = nchar(text)) %>% 
  group_by(ID_SALES) %>% 
  summarise(pol_min   = polarity %>% min(na.rm    = TRUE),
            pol_mean  = polarity %>% mean(na.rm   = TRUE),
...
finDat_test <- finDat_test %>% mutate(pol_min = as.numeric(pol_min),
                                      pol_min = ifelse(is.na(pol_min), 0, pol_min))

## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
finDat <- finDat %>% mutate(pol_mean = as.numeric(pol_mean),
                            pol_mean = ifelse(is.na(pol_mean), 0, pol_mean))
finDat_test <- finDat_test %>% mutate(pol_mean = as.numeric(pol_mean),
                                      pol_mean = ifelse(is.na(pol_mean), 0, pol_mean))
...
finDat_test <- finDat_test %>% mutate(pol_max = as.numeric(pol_max),
                                      pol_max = ifelse(is.na(pol_max), 0, pol_max))

## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
finDat <- finDat %>% mutate(sub_min = as.numeric(sub_min),
                            sub_min = ifelse(is.na(sub_min), 0, sub_min))
finDat_test <- finDat_test %>% mutate(sub_min = as.numeric(sub_min),
                                      sub_min = ifelse(is.na(sub_min), 0, sub_min))
...
finDat_test <- finDat_test %>% mutate(sub_mean = as.numeric(sub_mean),
                                      sub_mean = ifelse(is.na(sub_mean), 0, sub_mean))
...
finDat_test <- finDat_test %>% mutate(twl_max = as.numeric(twl_max),
                                      twl_max = ifelse(is.na(twl_max), 0, twl_max))
...
finMod_NT    <- h2o::h2o.upload_model(path = list.files(path = "2_output/3_model/bin_noTwitter/", full.names = TRUE))
finThresh_NT <- finMod_NT@model$default_threshold
...
trainDF <- trainDF %>%
  mutate(pred       = h2o::h2o.predict(object = finMod, newdata = train_h2o) %>% as_tibble() %>% pull(yes),
         ...
         pred_NT       = h2o::h2o.predict(object = finMod_NT, newdata = train_h2o) %>% as_tibble() %>% pull(yes),
         pred_class_NT = ifelse(pred_NT >= finThresh_NT, "yes", "no"))

testDF <- testDF %>%
  mutate(pred          = h2o::h2o.predict(object = finMod, newdata = test_h2o) %>% as_tibble() %>% pull(yes),
         pred_class    = ifelse(pred >= finThresh, "yes", "no"),
         pred_NT       = h2o::h2o.predict(object = finMod_NT, newdata = test_h2o) %>% as_tibble() %>% pull(yes),
         pred_class_NT = ifelse(pred_NT >= finThresh_NT, "yes", "no"))

finDat_test <- finDat_test %>%
  mutate(pred          = h2o::h2o.predict(object = finMod, newdata = finTest) %>% as_tibble() %>% pull(yes),
         pred_class    = ifelse(pred >= finThresh, "yes", "no"),
         pred_NT       = h2o::h2o.predict(object = finMod_NT, newdata = finTest) %>% as_tibble() %>% pull(yes),
         pred_class_NT = ifelse(pred_NT >= finThresh_NT, "yes", "no"))

write.csv(x = finDat_test %>% select(ID_SALES, pred_class), file = "2_output/1_data/finalTestPred.csv", row.names = FALSE)
...
rbind(trainDF, testDF) %>% filter(pred < 0.4, Retained == 1) %>% select(ID_SALES, sent_med, pol_med)
twiter$text[twiter$ID_SALES %in% (trainDF %>% filter(pred < 0.4, Retained == 1) %>% pull(ID_SALES))]

rbind(trainDF, testDF) %>% filter(pred > 0.6, Retained == 0) %>% select(ID_SALES, sent_med, pol_med) %>% print(n=46)
twiter$text[twiter$ID_SALES %in% (trainDF %>% filter(pred > 0.6, Retained == 0, sent_med > 0) %>% pull(ID_SALES))]
