...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
twiter <- readr::read_csv("2_output/1_data/sentTest.csv", locale = readr::locale(encoding = "latin1")) %>%
  mutate(tweetLength = nchar(text)) %>% 
  filter(set == "train") %>%
  group_by(ID_SALES) %>% 
  summarise(sent_med  = sentiment_clean    %>% median(na.rm = TRUE),
            pol_med   = polarity_clean     %>% median(na.rm = TRUE),
...
finDat <- finDat %>% mutate(sent_med = as.numeric(sent_med),
                            ...
                            sub_med  = as.numeric(sub_med),
                            sub_med  = ifelse(is.na(sub_med), 0, sub_med),
                            twl_med  = as.numeric(twl_med),
...
PerformanceAnalytics::chart.Correlation(finDat %>% select(starts_with("sent_")), histogram = TRUE)
...
ensemble <- h2o::h2o.stackedEnsemble(
  ...
  , training_frame        = train_h2o
  , model_id              = "ensemble"
  , base_models           = append(glmGrid@model_ids[1:3], gbmGrid@model_ids[1:3])
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
finMod    <- h2o::h2o.getModel("GBM_grid__1_AutoML_20210409_094250_model_1")
...
my_local_model <- h2o::h2o.download_model(model = finMod, export_cross_validation_predictions = TRUE)

# upload the model that you just downloaded above to the H2O cluster
uploaded_model <- h2o::h2o.upload_model(path = "2_output/3_model/bin/GBM_grid__1_AutoML_20210323_032348_model_14")

## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
finMod@model$model_summary
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(exm_train$varimp$plots[[1]], exm_test$varimp$plots[[1]], ncol = 2)


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(exm_train$shap_summary$plots[[1]], exm_test$shap_summary$plots[[1]], ncol = 2)


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(exm_train$pdp$plots$pol_med, exm_test$pdp$plots$pol_med, ncol = 2)


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(exm_train$pdp$plots$pol_min, exm_test$pdp$plots$pol_min, ncol = 2)


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(exm_train$pdp$plots$Program_Code, exm_test$pdp$plots$Program_Code, ncol = 2)
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(lr$ice$plots$pol_mean, lr$ice$plots$pol_med, lr$ice$plots$Program_Code, ncol = 3)


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
mr <- h2o::h2o.explain_row(finMod, test_h2o, row_index = which(testDF$pred>0.545&testDF$pred<0.551)[1])


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
testDF[which(testDF$pred>0.54&testDF$pred<0.56)[1], ] %>% select(pred, Retained, pred_class)
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(mr$ice$plots$pol_med, mr$ice$plots$pol_min, mr$ice$plots$Program_Code, ncol = 3)
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
gridExtra::grid.arrange(hr$ice$plots$pol_mean, hr$ice$plots$Income_Level, hr$ice$plots$Program_Code, ncol = 3)
